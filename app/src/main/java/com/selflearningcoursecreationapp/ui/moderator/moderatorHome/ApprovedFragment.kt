...
class ApprovedFragment : BaseFragment<FragmentApprovedBinding>() {
    ...
    private val adapter by lazy {
        AdapterAcceptedRejectedList(viewModel, MODSTATUS.ACCEPTED) { type, wishListItem, _ ->
            when (type) {
                0 -> {
...
//                    }.show(childFragmentManager, "")
    ...
    private fun initUI() {
        ...
        hashMap["PageSize"] = 20
        hashMap["status"] = MODSTATUS.ACCEPTED
        viewLifecycleOwner.lifecycleScope.launch {
            viewModel.acceptCourse(hashMap).observe(viewLifecycleOwner) {
