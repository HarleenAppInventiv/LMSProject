...
class RangeSeekBarView : View {
    ...
    private val paddingOnTop: Int = 0
    ...
    private val whiteColorRes = context.resources.getColor(R.color.trimmer_line_color)
    ...
    private fun init() {
        ...
        thumbImageLeft = BitmapFactory.decodeResource(resources, R.drawable.ic_trim_start)
        thumbImageRight = BitmapFactory.decodeResource(resources, R.drawable.ic_trim_end)
        ...
        val newWidth: Int = context.resources.getDimensionPixelOffset(R.dimen._20sdp)
        val newHeight: Int = /*UnitConverter.dpToPx(context, */
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        val leftRect = Rect(bg_middle_left.toInt(), height, (rangeL + 20).toInt(), 0)
        val rightRect = Rect((rangeR - 20).toInt(), height, (bg_middle_right).toInt(), 0)
        canvas.drawRect(leftRect, mShadow)
        canvas.drawRect(rightRect, mShadow)
        if (rectPaint != null) {
            canvas.drawRect(
                rangeL + UnitConverter.dpToPx(context, 10).toFloat(),
                thumbPaddingTop + paddingOnTop,
                rangeR - UnitConverter.dpToPx(context, 10).toFloat(),
                thumbPaddingTop + UnitConverter.dpToPx(context, 2) + paddingOnTop,
                rectPaint!!
            )
            canvas.drawRect(
                rangeL + UnitConverter.dpToPx(context, 10).toFloat(),
                height.toFloat() - UnitConverter.dpToPx(context, 2).toFloat(),
                rangeR - UnitConverter.dpToPx(context, 10).toFloat(),
                height.toFloat(),
                rectPaint!!
            )
